...
describe('TaskService', () => {
  ...
  describe('reorder', () => {
    ...
    it('should throw ForbiddenException for another user workspace', async () => {
      ...
      await expect(
        service.reorder(user2.id, workspace.id, { taskIds: ['some-id'] }),
      ).rejects.toThrow(ForbiddenException);
    });
  });
});
