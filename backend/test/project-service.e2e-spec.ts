...
describe('ProjectService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should include subjects in disciplines', async () => {
      ...
      const discipline = await prisma.project.create({
      ...
      expect(disciplines[0].tasks).toHaveLength(2);
      expect(disciplines[0].tasks[0].name).toBe('Grammar');
      expect(disciplines[0].tasks[1].name).toBe('Vocabulary');
    });
    ...
    it('should not include archived subjects', async () => {
      ...
      expect(disciplines[0].tasks).toHaveLength(1);
      expect(disciplines[0].tasks[0].name).toBe('Active');
    });
  ...
  describe('findOne', () => {
    it('should return discipline with subjects', async () => {
      ...
      expect(result.tasks[0].name).toBe('Algebra');
    });
  ...
  describe('create', () => {
    ...
    it('should create discipline with linked subjects', async () => {
      ...
      const subject1 = await prisma.task.create({
        data: { workspaceId: workspace.id, name: 'Algebra', position: 0 },
      });
      const subject2 = await prisma.task.create({
        data: { workspaceId: workspace.id, name: 'Geometry', position: 1 },
      });

      const discipline = await service.create(user.id, workspace.id, {
        name: 'Math',
        taskIds: [subject1.id, subject2.id],
      });

      expect(discipline.tasks).toHaveLength(2);
    });
  ...
  describe('update', () => {
    ...
    it('should update subject assignments', async () => {
      ...
      expect(updated.tasks[0].id).toBe(subject2.id);

      // Verify subject1 is now unlinked
      const unlinkedSubject = await prisma.task.findUnique({
    ...
    it('should throw ConflictException for duplicate name', async () => {
      ...
      const discipline2 = await prisma.project.create({
  ...
  describe('addTasks', () => {
    it('should add subjects to discipline', async () => {
      ...
      const updated = await service.addTasks(user.id, discipline.id, [
        subject.id,
      ]);

      expect(updated.tasks).toHaveLength(1);
      expect(updated.tasks[0].id).toBe(subject.id);
    });
  ...
  describe('removeTasks', () => {
    it('should remove subjects from discipline', async () => {
      ...
      const updated = await service.removeTasks(user.id, discipline.id, [
        subject.id,
      ]);
    ...
    it('should throw BadRequestException for empty taskIds', async () => {
      ...
      await expect(
        service.removeTasks(user.id, discipline.id, []),
  ...
  describe('reorder', () => {
    it('should reorder disciplines', async () => {
      ...
      const d1 = await prisma.project.create({
        data: { workspaceId: workspace.id, name: 'First', position: 0 },
      });
      const d2 = await prisma.project.create({
        data: { workspaceId: workspace.id, name: 'Second', position: 1 },
      });
      const d3 = await prisma.project.create({
