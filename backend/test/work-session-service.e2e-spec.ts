...
import { createTestUser, createTestUserWithWorkspace, createTestWorkspace, createTestTask } from './helpers/database.helper';
...
describe('WorkSessionService', () => {
  ...
  describe('findByDateRange', () => {
    ...
    it('should filter by start date', async () => {
      ...
      expect(sessions[0].task.name).toBe('Physics');
    });
    ...
    it('should filter by end date', async () => {
      ...
      expect(sessions).toHaveLength(1);
      expect(sessions[0].task.name).toBe('Math');
    });
    ...
    it('should filter by date range', async () => {
      ...
      expect(sessions).toHaveLength(1);
      expect(sessions[0].task.name).toBe('Chemistry');
    });
    ...
    it('should order sessions by date ascending', async () => {
      ...
      const sessions = await service.findByDateRange(user.id, workspace.id);
      expect(sessions[0].task.name).toBe('Math');
      expect(sessions[1].task.name).toBe('Chemistry');
      expect(sessions[2].task.name).toBe('Physics');
    });
    ...
    it('should return all sessions when workspaceId is "all"', async () => {
      ...
      const physics = await createTestTask(workspace2.id, { name: 'Physics' });
  ...
  describe('update', () => {
    ...
    it('should update session subject', async () => {
      const { user, workspace } = await createTestUserWithWorkspace();
      const math = await createTestTask(workspace.id, { name: 'Math' });
      const algebra = await createTestTask(workspace.id, { name: 'Algebra' });
      const session = await service.create(user.id, {
        workspaceId: workspace.id,
        date: '2024-12-16',
        subjectId: math.id,
      ...
      expect(updated.task.name).toBe('Algebra');
  ...
  describe('getDistinctTasks', () => {
    ...
    it('should return subjects from all workspaces when workspaceId is "all"', async () => {
      ...
      const subjects = await service.getDistinctTasks(user.id, 'all');
  ...
  describe('User isolation', () => {
    it('should not return sessions from other users', async () => {
      ...
      expect(user1Sessions[0].task.name).toBe('Math');

      expect(user2Sessions).toHaveLength(1);
      expect(user2Sessions[0].task.name).toBe('Physics');
    });
    ...
    it('should not return subjects from other users', async () => {
      ...
      const user1Subjects = await service.getDistinctTasks(user1.id, 'all');
      const user2Subjects = await service.getDistinctTasks(user2.id, 'all');
